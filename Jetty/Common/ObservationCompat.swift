...
// Copied from Top Drawer's MacDring/Common/ObservationCompat.swift rather than shared:
// keep the two in sync by hand. See docs/linux-port-plan.md §JP-02.
//
// The types are named to match Combine's so the shared code (`DockStore`, `WeatherService`,
// …) reads identically on both platforms; they live in the `Jetty` module rather
