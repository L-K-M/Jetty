...
final class DisplayRegistry: ObservableObject {
    ...
    func rebuild() {
        ...
        for screen in screens {
            ...
            var n = 2
            // A display must always be able to reclaim its own base UUID (`uniqueKey ==
            // baseKey`): the reservation only protects collision *suffixes* (`U#2`) from
            // being stolen. Without the `!= baseKey` guard, a unique-UUID display that
            // reconnects with a fresh CGDirectDisplayID (so `previousKey` is nil) would
            // find its own base key "reserved" and get pushed to `U#2`, losing its
            // per-display dock settings — the exact case UUIDs exist to survive.
            while map[uniqueKey] != nil
                    || (reservedKeys.contains(uniqueKey) && uniqueKey != previousKey && uniqueKey != baseKey) {
                uniqueKey = "\(baseKey)#\(n)"
                n += 1
            }
