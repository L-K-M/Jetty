...
extension NSApplication {
    /// Reverts Jetty to its menu-bar agent policy (`.accessory`, no Dock icon),
    /// **unless another ordinary window is still visible**.
    ///
    /// Jetty is an `.accessory` app; it switches to `.regular` while a titled window
    ...
    /// the Dock presence the open one needs. The borderless, non-activating dock /
    /// Jetty-Menu panels can't become main, so they're never counted.
