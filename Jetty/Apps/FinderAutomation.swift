...
enum FinderAutomation {
    ...
    static func permissionStatus() -> Status {
        ...
        defer { AEDisposeDesc(&address) }   // AECreateDesc allocates; release it (H-review)
    ...
    static func trashItemCount(completion: @escaping (Int?) -> Void) {
        AppleScriptRunner.run(#"tell application "Finder" to get (count of items of trash)"#) { result, _ in
            // `result?.int32Value` would bind `.map` to the non-optional Int32; map the
            // optional descriptor instead. nil result = denied/error → nil count.
            completion(result.map { Int($0.int32Value) })
        }
    }
